...
namespace GRUPA_K13.Classes.System
{
    public sealed class Log : IDisposable
    {
        ...
        private string GenerateLogShiftRightLine(string a_sText, bool a_bIsHeader, LevelEnum a_eLevel)
        {
            return $"{DateTime.Now:dd/MM/yyyy HH:mm:ss} <{a_eLevel}> {IteratedLineShift(a_bIsHeader)}{a_sText}";
        }

        private string GenerateLogShiftLeftLine(LevelEnum a_eLevel)
        {
        ...
        private void Print(string a_sText, bool a_bIsHeader, LevelEnum a_eLevel)
        {
            lock (LockObject)
            {
                ...
                {
                    LogWriter?.Write(GenerateLogShiftRightLine(a_sText, a_bIsHeader, a_eLevel));
                }
            }
        }
        ...
        public void Dispose()
        {
            lock (LockObject)
            {
                if (m_bLineShift)
                {
                    LogWriter?.Write(GenerateLogShiftLeftLine(m_eLevel));
                    Log.Iteration--;
                }
